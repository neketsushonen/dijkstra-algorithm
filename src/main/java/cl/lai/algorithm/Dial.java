...
public class Dial 
{
    ...
    public static void main( String[] args ) throws Exception {
    	...
    	while((aux=reader.readLine())!=null) {
    		...
    		vertices.get(Integer.parseInt(array[0])).aristas.add(new Arista(vertices.get(Integer.parseInt(array[1])), Integer.parseInt(array[2])));
    		...
    		for(int j=1;j<=nodos;j++) {
    			if(i==j)
    				writer.println(i+" "+i+" "+0);
    				 ...
    				 try {
    					 List<Vertice> vtx = getShortestPathTo(vertices.get(j));
    					 writer.println(j+" "+vtx.get(vtx.size()-2)+" "+vertices.get(j).distanciaMinima.intValue());
